...
pub fn three_cliques<N: NodeTrait, E>(graph: &UnGraphMap<N, E>) -> HashSet<[N; 3]> {
    let mut cliques = HashSet::new();
    for (u, v, _) in graph.all_edges() {
        for w in graph.neighbors(u) {
            if graph.contains_edge(v, w) {
                let mut three_clique = [u, v, w];
                three_clique.sort();
                cliques.insert(three_clique);
            }
        }
    }
    cliques
...
/// Copied from https://github.com/petgraph/petgraph/pull/662
pub mod maximal_cliques {
        ...
        G::NodeId: Eq + Hash,
    {
        ...
        let mut todo = p
            .iter()
            .filter(|&v| {
                *u == *v || !g.is_adjacent(adj_mat, *u, *v) || !g.is_adjacent(adj_mat, *v, *u)
